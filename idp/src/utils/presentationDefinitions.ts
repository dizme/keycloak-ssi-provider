...
function getCustomPresentation(id: string, credentialType: string, credentialName: string = credentialType, attributes: string[]) {
    ...
    // Check if specific attributes are required, otherwise skip
    if (attributes.length) {
        // Add constraints field only if filteredAttributes is not empty
        newPresentation.input_descriptors[0].constraints = {
